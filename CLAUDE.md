...
- **Test hubs get their configuration per host, never through the process environment.** The hub reads its variables through `builder.Configuration`, whose only source in a deployment is the environment; `tests/support/GrimoireHub.cs` hands the same names to each host with `UseSetting`. That is what lets the C# suites run their tests in parallel. A test that sets `Environment.SetEnvironmentVariable` for a hub brings back the failures that move between runs, because every hub in the process then shares the last one's wiki. Parallelism is capped at half the cores per suite by the one `tests/xunit.runner.json`: `dotnet test` runs every suite at once, and uncapped, runs slowed past the suites' timeouts.
