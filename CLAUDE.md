...
| Why the stack is what it is (`DEC-001`…`DEC-025`) | `docs/decisions.md` |
...
| How the current feature came about | `specs/003-live-run-record/` — `spec.md`, `plan.md`, `tasks.md`, `research.md`, `data-model.md`, `contracts/`, `quickstart.md`. Closed before it: `specs/001-first-ingest/`, `specs/002-ingest-queue/` |
...
- **`src/Grimoire.Runs`** — what a submission and a run *are*. `SubmissionBoard` accepts or refuses a text (one run at a time, both start-up inputs present, non-empty); `Submission` holds one of four states behind the board's single lock; `Run` (`RunStateMachine.cs`) makes every judgment about a run — the nudge decision and the final verdict. `ISubmissionStore` is its port to where the submissions survive a stop, with `Adapters/SqliteSubmissionStore.cs` the only file in the tree that names SQLite (RUNS-004, DEC-023); its members are synchronous, because the board writes each change under the one lock it decides the queue rule with.
...
- **`src/Grimoire.Hub`** — the only project that knows all three. `Program.cs` reads the arguments and puts the three real adapters at their ports — the wiki store, the `claude` harness and the submission store; `HubApplication.Build` is what every suite builds too, with in-memory adapters (III.9). It serves the static page from `wwwroot/` (no bundler, DEC-019), `POST`/`GET /api/submissions` and `POST /api/submissions/{id}/acknowledgement`, and the five wiki tools over MCP at `/mcp/runs/{runId}` — unauthenticated, loopback only (DEC-014). `InstructionLoader` is the *only* thing that puts text into the agent's prompt (V.1); `SubmissionIntake` accepts and dispatches without the user waiting; `RunQueue` asks the board for the next run and dispatches it, pumped by the four events that can unblock the queue and by no timer; `RunConductor` holds what happens while a run is under way.
...
Around that, since `002-ingest-queue`: a text is accepted whatever is running and waits its turn, one run at a time in the order the submissions were made (RUNS-002); a run that ended failed holds the queue until the user acknowledges it (RUNS-003); the submissions and their states survive any stop, a kill or a power cut included, and a run that was in progress reads failed afterwards (RUNS-004); and no agent goes on working on a run Grimoire has ended — stopped with the hub where it can act, terminated at the next start-up by the process identity recorded with the run where it could not (RUNS-006, DEC-024).
