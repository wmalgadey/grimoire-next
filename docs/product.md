...
One person, one wiki with a purpose described by the user, self-hosted, used in the browser. Sources are submitted in passing, several times a week. The user reviews the results afterwards. During a run they do not wait. They also read and edit the wiki outside of Grimoire.

The user describes the purpose of the wiki in a hand-written file inside the wiki itself. Every run receives it. Grimoire neither creates nor modifies it.
...
- No judgement of content by Grimoire: judgement about content rests with the agent and the
  ...
  Which source carries which statement is recorded by the agent.
- Undo happens through the wiki's version control, not through Grimoire.
- Not multiple wikis per instance.
...
| WIKI       | Shape of the wiki as an artifact: format, linking, provenance, entry point          |
| RUNS       | Traceability: what was done, why, cost; approving lint tasks                        |
| ACCESS     | Access paths and users                                                              |
...
| OUT-14 | have an assistant draft the description of the wiki                                        | Later  | Wiki           |       |
