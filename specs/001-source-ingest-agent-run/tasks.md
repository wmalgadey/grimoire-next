...
- [X] T144 Reset the working tree in `WikiMutation.RevertIfStillTip` when `git revert` or the restoring commit fails, and answer with a problem response rather than an unhandled 500, with a test, per FR-025, Constitution II.1 (missing)
- [X] T145 Add a TS-10 case writing `.git/hooks/post-commit` and `.git/config` through `mcp__wiki__write_page`, asserting both are refused, recorded, and the files untouched, per FR-011, TS-10 (missing)
- [X] T146 Record a commit even when the post-commit clean or metadata read in `WikiMutation.CommitRun`/`GitCli.CommitAll` fails — return it, or log it critical — so it never becomes a commit no task accounts for, per FR-015, SC-003 (partial)
- [X] T147 Take the commit message from the last assistant turn only, so an empty final turn falls back to `ingest <taskId>` instead of an earlier narration message (`src/agentrun/src/model/adapter.ts`), per research R9, contracts/runner-protocol.md "run_end" (contradicts)
- [X] T148 Decode a retrieved body by its declared charset instead of always as UTF-8 (`src/ingest/adapters/UrlFetch.cs`), per FR-029 (contradicts)
