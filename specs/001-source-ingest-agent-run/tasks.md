...
- [ ] T141 Lift Kestrel's default request-body limit on `POST /api/tasks` so pasted text over ~30 MB becomes a task instead of a 413, with an ingest test above the limit expecting 201, per FR-029 (contradicts)
- [ ] T142 Put an overall deadline on URL retrieval, the body read included (`HttpClient.Timeout` does not cover it after `ResponseHeadersRead`), failing the task with a reason, with a TS-03 case whose origin trickles its body, per FR-003 (partial)
- [ ] T143 Kill the runner process on every non-success exit from `RunnerProcess.Run` (a protocol error or a store failure mid-run leaves it writing into the tree), and reset the working tree before each dispatch, with a test that emits a malformed line mid-run, per FR-017, SC-003 (partial)
- [ ] T144 Reset the working tree in `WikiMutation.RevertIfStillTip` when `git revert` or the restoring commit fails, and answer with a problem response rather than an unhandled 500, with a test, per FR-025, Constitution II.1 (missing)
- [ ] T145 Add a TS-10 case writing `.git/hooks/post-commit` and `.git/config` through `mcp__wiki__write_page`, asserting both are refused, recorded, and the files untouched, per FR-011, TS-10 (missing)
- [ ] T146 Record a commit even when the post-commit clean or metadata read in `WikiMutation.CommitRun`/`GitCli.CommitAll` fails — return it, or log it critical — so it never becomes a commit no task accounts for, per FR-015, SC-003 (partial)
- [ ] T147 Take the commit message from the last assistant turn only, so an empty final turn falls back to `ingest <taskId>` instead of an earlier narration message (`src/agentrun/src/model/adapter.ts`), per research R9, contracts/runner-protocol.md "run_end" (contradicts)
- [ ] T148 Decode a retrieved body by its declared charset instead of always as UTF-8 (`src/ingest/adapters/UrlFetch.cs`), per FR-029 (contradicts)
- [ ] T149 Tighten gate 2 so `Microsoft.Data.Sqlite`, `System.Diagnostics.Process` and `HttpClient` are confined to adapter namespaces, not whole slices, and move the per-run home-directory handling out of `Dispatcher` into `RunnerProcess`, per Constitution V.2, V.3 (partial)
- [ ] T150 Strengthen TS-14/TS-19: queue two or more tasks across a restart and assert their dispatch order, assert no model request for a recovered task after restart, and assert the dirty tree before `SIGTERM` and the runner's exit after it, per FR-028, TS-14, TS-19 (partial)
- [ ] T151 Assert in TS-16 that a running task shows the tool calls recorded so far and that a queued task appears as queued in the task list, per US3/AC1, US3/AC2 (partial)
