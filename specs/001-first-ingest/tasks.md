...
- [ ] T024 [P] [US2] In `tests/Grimoire.Contract.Tests/FileSystemWikiStoreTests.cs`: `FileSystemWikiStore` against a real wiki directory on disk — the stamped page as written, an unreadable frontmatter refused, a failed run's files still there, a path escaping the wiki root refused with `outside-wiki`, which is the spec's edge case for GUARD-001 — **Req:** GUARD-001, WIKI-002, WIKI-003 | **Level:** Contract — **Why not lower:** Fast cannot prove it — the real filesystem decides what lands on disk and how a path resolves, and that is the external thing at this port
