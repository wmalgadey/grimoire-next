...
- [ ] T003 [P] Add the CI workflow in `.github/workflows/ci.yml`: build; `dotnet test tests/Grimoire.Fast.Tests -- --timeout 15s`; `dotnet test tests/Grimoire.Contract.Tests -- --timeout 90s --filter-not-trait "requires=signin"`; then `trace-check`. The marker is named once, here — **Req:** Principle III.7
...
- [ ] T004 Register this feature's sixteen requirements in `docs/capabilities/{ingest,wiki,guard,access,runs}.md` with their proof kinds. **This is the first implementation task: it comes before any test is written.** RUNS-002, RUNS-003, RUNS-004 and ACCESS-003 are not registered — they belong to the follow-up feature — **Req:** Principle IV.2
...
- [ ] T009 [P] [US1] In `tests/Grimoire.Fast.Tests/SubmissionRefusalTests.cs`: a missing purpose description refuses with `purpose-description-missing`, and a text that is empty or only whitespace after trimming refuses with `text-empty`; neither stores a submission nor starts a run, and the purpose description is checked first — **Req:** INGEST-003, INGEST-004 | **Level:** Fast — **Why not lower:** there is no lower level
- [ ] T010 [P] [US1] In `tests/Grimoire.Fast.Tests/SubmissionStateTests.cs`: a submission carries exactly one state at a time, drawn from `Submitted` · `Running` · `Done` · `Failed`, across every transition of data-model.md; `Done` and `Failed` are terminal — **Req:** RUNS-001 | **Level:** Fast — **Why not lower:** there is no lower level
...
- [ ] T015 [US1] `src/Grimoire.Hub/Api/SubmissionsEndpoints.cs`: `POST /api/submissions` exactly as `contracts/hub-http-api.md` specifies — `202` with `{id,state,submittedAt}`; `422 purpose-description-missing`; `422 text-empty`; `409 run-in-progress`. A refused submission is stored nowhere and carries no state — **Req:** INGEST-001, INGEST-003, INGEST-004, INGEST-005
...
- [ ] T018 [P] [US2] In `tests/Grimoire.Fast.Tests/DispatchPayloadTests.cs`: a dispatched run is given the instruction, the purpose description, the submitted text and the run's identifier, and nothing else reaches the prompt — **Req:** INGEST-002 | **Level:** Fast — **Why not lower:** there is no lower level; the payload is assembled by our own objects
...
- [ ] T021 [P] [US2] In `tests/Grimoire.Fast.Tests/ToolGrantTests.cs`: the per-run endpoint serves exactly `list_pages`, `read_page`, `write_page`, `write_index`, `append_log` — reading anything in the wiki, creating and changing pages, indexes and the log, and nothing else; no delete and no move; a name outside the grant has no handler; and the grant is recorded with the run — **Req:** GUARD-001, GUARD-002, GUARD-003 | **Level:** Fast — **Why not lower:** there is no lower level for the hub's half of the grant; the CLI's half is T025
- [ ] T022 [P] [US2] In `tests/Grimoire.Fast.Tests/CeilingTests.cs`, driven by `FakeTimeProvider`: no further model call starts once either ceiling is reached; the elapsed-time ceiling stops a call in flight; a ceiling ends the run failed; both ceilings are fixed values counted in model tokens and elapsed time, not settings — **Req:** GUARD-004 | **Level:** Fast — **Why not lower:** there is no lower level, and a fake clock keeps the Fast suite inside its 15 s budget
- [ ] T023 [P] [US2] In `tests/Grimoire.Fast.Tests/RunOutcomeTests.cs`, the decision table of `contracts/harness-protocol.md`: agent stopped with the entry present and both ceilings clear → done; stopped with no entry and not yet nudged → nudged once, still running; stopped with no entry and already nudged → failed; a ceiling reached → failed whatever the log says. Nothing but the log entry is read to decide — **Req:** RUNS-005 | **Level:** Fast — **Why not lower:** there is no lower level; the decision is ours, and the log lookup goes through the wiki port
- [ ] T024 [P] [US2] In `tests/Grimoire.Contract.Tests/FileSystemWikiStoreTests.cs`: `FileSystemWikiStore` against a real wiki directory on disk — the stamped page as written, an unreadable frontmatter refused, a failed run's files still there, a path escaping the wiki root refused with `outside-wiki` — **Req:** WIKI-002, WIKI-003 | **Level:** Contract — **Why not lower:** Fast cannot prove it — the real filesystem decides what lands on disk and how a path resolves, and that is the external thing at this port
- [ ] T025 [US2] In `tests/Grimoire.Contract.Tests/HarnessProcessTests.cs`, marked `[Trait("requires","signin")]`: one real run of the real `claude` CLI against a real wiki in a temporary directory — `system/init` reports a `tools` array equal to the grant and the `wiki` server connected; a granted tool writes; and a run asked for something only a shell or a file tool could do makes no call outside the grant and changes nothing outside the wiki — **Req:** GUARD-001, GUARD-002 | **Level:** Contract — **Why not lower:** Fast cannot prove it — the tool surface is decided by the real CLI at startup, and whether our deny-by-default configuration actually holds is exactly what this checks
...
- [ ] T030 [P] [US2] `src/Grimoire.Agent/Grant.cs` and `Ceilings.cs`: the five granted tool names recorded with the run, and the two fixed ceilings measured against `TimeProvider` — **Req:** GUARD-002, GUARD-003, GUARD-004
- [ ] T031 [US2] `src/Grimoire.Agent/Adapters/HarnessProcess.cs`: the only place the `claude` process and its NDJSON live. Spawns it with exactly the argv of `contracts/harness-protocol.md` — `--tools ""`, `--mcp-config` + `--strict-mcp-config`, `--allowed-tools "mcp__wiki__*"`, `--permission-mode dontAsk`, `--setting-sources ""`, `--include-partial-messages`, `--input-format stream-json`, `--no-session-persistence` — in a working directory Grimoire owns, not the wiki. Asserts `system/init`'s tool list equals the grant, totals `usage` off `message_delta`, reconciles against the `result` message, sends the nudge as a further user message and the stop as a `control_request` interrupt, and kills the process only as a backstop — **Req:** GUARD-001, GUARD-004, Principle V.2
...
- [ ] T035 [US3] In `tests/Grimoire.E2E.Tests/SubmissionStatesTests.cs`: with submissions in different states, a real browser shows each one as exactly one of submitted, running, done, failed, and shows no step, reasoning, duration, cost or history — **Req:** ACCESS-002 | **Level:** E2E — **Why not lower:** Contract cannot prove it — the requirement is about what the browser shows, including what it must *not* show
...
- [ ] T038 Run `trace-check`; it passes — every `test` requirement has a test carrying its ID, no test carries an unknown, retired or reserved ID, every test has a level, and every E2E test has a requirement ID — **Req:** Principle IV.3
...
- [ ] T041 Walk `docs/review-checklist.md`, all twelve items — including item 4's question whether the owner has read what WIKI-001, this feature's one review-proven requirement, is about — **Req:** Principle Gov.2, Principle I.9
