...
- [X] T003 [P] Add the CI workflow in `.github/workflows/ci.yml`: build; `dotnet test tests/Grimoire.Fast.Tests -- --timeout 15s`; `dotnet test tests/Grimoire.Contract.Tests -- --timeout 90s --filter-not-trait "requires=signin"`; then `trace-check`. The marker is named once, here — **Req:** Principle III.7
...
- [ ] T004 Register this feature's sixteen requirements in `docs/capabilities/{ingest,wiki,guard,access,runs}.md` with their proof kinds. **This is the first implementation task: it comes before any test is written.** The four requirements the split moved are not registered here — they belong to the follow-up feature — **Req:** Principle IV.2
...
- [ ] T015 [US1] `src/Grimoire.Hub/Api/SubmissionsEndpoints.cs`: `POST /api/submissions` exactly as `contracts/hub-http-api.md` specifies — `202` with `{id,state,submittedAt}`; `422 purpose-description-missing`; `422 text-empty`; `409 run-in-progress`. The first of those is the **purpose description absent → refuse** check, made against the path the hub was started with and before the text is looked at. A refused submission is stored nowhere and carries no state — **Req:** INGEST-001, INGEST-003, INGEST-004, INGEST-005
...
- [ ] T021 [P] [US2] In `tests/Grimoire.Fast.Tests/ToolGrantTests.cs`: the per-run endpoint serves exactly `list_pages`, `read_page`, `write_page`, `write_index`, `append_log` — reading anything in the wiki, creating and changing pages, indexes and the log, and nothing else; no delete and no move; a name outside the grant has no handler; and the grant is recorded with the run. Then, through the in-memory adapter at `IAgentHarness`: an agent that **reports a tool outside the grant** makes the run end failed **before its first model call**, and no dispatch follows — **Req:** GUARD-001, GUARD-002, GUARD-003 | **Level:** Fast — **Why not lower:** there is no lower level for the hub's half of the grant; that the real CLI reports the surface it was given is T025
