...
- Q: Which review item proves WIKI-001? → A: Review-checklist item 3. **Carried further in the session below**: item 3 as it stood asked only whether the standard's named parts are built, which covers WIKI-001's root-index clause and none of the rest, so it was widened by amendment rather than read more broadly than it was written. That amendment has landed.
...
### Session 2026-09-21 — after the first real ingest

- Q: `generated.by` was written as `Grimoire; model <id>; run <guid>`, which is free text. OKF 0.2 §7 admits three spellings of an actor and no others — `<producer>/<version>`, `human:<id>`, `process:<id>` — and the root index we write declares `okf_version: "0.2"`. What gives way? → A: The record does. Grimoire writes `grimoire/<model>`, the `<producer>/<version>` form. **The run identifier leaves the page with it**: an actor is one actor, and the page was produced by the agent rather than by a run. WIKI-002's closing clause therefore reads "the record MUST carry that run's values" rather than "MUST name that run" — which is what the requirement was protecting anyway, that an update does not leave an earlier run's record standing. The edge case is reworded to match. Nothing is renumbered and no proof kind changes; no test asserted the identifier in the record, because `ProvenanceStamp` is agnostic about the string it is given.
- Q: Does anything still tie a page to the run that wrote it? → A: The log entry, which since this session names every file the run touched, and `generated.at`, which dates the page. The tie now runs log → page instead of page → log. OWNER DECISION: that is enough, and carrying a GUID on every page to keep the other direction is not worth it.
- Q: Should `generated.by` name the person who submitted the text? → A: No. §7 allows exactly one actor, and the producer of the page is the agent, not the person; `human:<id>` there would claim the person wrote it. Grimoire also has no notion of user identity — one user, trusted network (`docs/product.md` §2) — so the name would be a constant with no consumer (Constitution II.1). It becomes a real question with OUT-11 and OUT-12, where more than one person submits.
...
- Q: Item 3 of `docs/review-checklist.md` is "Standard scope" and does not ask what the instruction says, so WIKI-001 had no item that actually proves it (Constitution III.1). How is it settled? → A: Item 3 is widened to ask both — that only the standard's named parts are built, and that the instruction every run receives states the shape those requirements demand. Twelve items stand; no thirteenth, so nothing is retired. **The widening was an amendment and therefore its own PR, touching only the constitution, the template overrides and the checklist (Governance 4); it is no part of this feature.** That PR has landed: item 3 now reads "Standard scope and the instruction" and asks, where a capability requirement places the shape of an artifact on the agent rather than on our code, whether the instruction every run receives states that shape in full. WIKI-001 is proven by item 3 as it now stands.
...
- Q: Which model does a run use, and where does that come from? → A: From Grimoire's start-up inputs, beside the wiki and the purpose description: one pinned model id, given to the hub when it starts and used by every run it dispatches. Carried by INGEST-002, so the payload the dispatch test reads proves it. The plan's Technology decisions already ruled out an alias and the CLI's default — both follow something outside this repository — and a start-up input is what keeps the choice with the owner instead of with the machine or the submission.
- Q: INGEST-003 refuses a submission when the purpose description is missing, but the instruction reaches the prompt by the same route and its absence is just as fatal — what happens then? → A: The same thing. INGEST-003 now covers both, and the refusal says which of the two is missing so the user knows which file to put back. Both are read from the paths the hub was started with, before the text is looked at; neither absence stores a submission nor starts a run.
...
| --- | --- | --- |
| The instruction or the purpose description is missing when a text is submitted | The submission is refused, the user is told which of the two is missing, no run starts, and nothing is stored or listed. | INGEST-003 |
...
| A run updates a page an earlier run wrote | The record on that page carries the updating run's values, never the earlier run's. | WIKI-002 |
...
- **SC-002**: Every run receives the instruction, the purpose description, the submitted text and its own identifier, and runs on the model Grimoire was started with. — **Restates:** INGEST-002
...
- The instruction is a versioned text belonging to Grimoire, distinct from the user-written purpose description. Both are start-up inputs, read from the paths the hub was started with, and both go to every run; the instruction is where the wiki's shape is demanded (Constitution V.1). — **Requirement:** INGEST-002, INGEST-003, WIKI-001
- The model is a start-up input too: the hub is given one pinned model id and every run it dispatches uses it, never an alias and never the CLI's own default. Which id that is, is the owner's choice and not a per-submission one. — **Requirement:** INGEST-002
