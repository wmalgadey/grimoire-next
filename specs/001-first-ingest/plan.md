...
| How tests are named | A test class is `<Subject>Tests`, the subject an entity from the spec's vocabulary; a test method is `<Action>_<Result>[_<Scenario>]`, PascalCase parts separated by one underscore, the scenario present wherever the result depends on a condition and opening with **When / While / With / Without / After**. Names use the spec's vocabulary, never the implementation's — no status codes, no method names; the result is an observable outcome, never `Works` or `Correctly`; a name that needs `And` is two tests. Requirement IDs live in the `req` trait and never in a name. Written down once in `tests/README.md` | A test name is read in `docs/trace.md` and in CI failure output, where one glance has to show which behaviour broke and under which condition. Taking the subject from the spec rather than from a class or method means a rename in the code cannot make a name wrong, which is what keeps a test tied to the requirement it proves rather than to the code that happens to implement it (III.1). CA1707 forbids the underscores and is switched off for the test projects alone, in `tests/.editorconfig` (research.md R-13) | yes | none |
...
| Complexity ceiling | The SDK's own cyclomatic-complexity rule, CA1502, as an **error** at a threshold of **15** for everything under `src/` and `tools/`; the suites are excluded. An absolute ceiling, not a delta against a baseline | An absolute ceiling needs no baseline file and no tooling of our own to keep one (II.3) — the threshold is four lines of configuration the analyzer already reads. Set while the code is small it never needs an exception list: a method that would exceed it is refactored rather than excused. This is not a gate in the constitution's sense (II.2) and gets no CI job: the build already fails on analyzer errors, which T002 established (research.md R-14) | yes | none |
...
Only `001-first-ingest` merges to `main`, and only when the feature is done (I.9) — every task
complete, both gates green, `docs/trace.md` regenerated and the owner's acceptance run behind it.

| Phase | Tasks | Branch | PR |
| --- | --- | --- | --- |
| 1 Setup | T001–T003 | `001-first-ingest-phase-1-setup` | #20 |
| 2 Foundational | T004–T007 | `001-first-ingest-phase-2-foundation` | #23 |
| 3 User Story 1 | T008–T017 | `001-first-ingest-phase-3-submit` | not opened yet |
| 4 User Story 2 | T018–T034 | `001-first-ingest-phase-4-wiki` | not opened yet |
| 5 User Story 3 | T035–T036 | `001-first-ingest-phase-5-states` | not opened yet |
| 6 Closing the feature | T037–T042 | `001-first-ingest-phase-6-close` | not opened yet |

Every PR targets `001-first-ingest`. It stays a draft until its phase is complete and leaves the
build and the test suites green on the feature branch. A step that cannot pass yet belongs in the
PR that makes it pass: T003's `trace-check` steps are in phase 2 with the tool that runs them, not
in phase 1 where the task is listed.
...
(`dotnet run --project src/Grimoire.Hub -- --wiki … --purpose … --model …`), open the submission page, paste
